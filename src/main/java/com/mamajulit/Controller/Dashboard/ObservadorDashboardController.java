package com.mamajulit.Controller.Dashboard;

import com.mamajulit.View.Dashboard.ObservadorDashboardView;
import javax.swing.*;
import java.awt.*;

public class ObservadorDashboardController {

    private ObservadorDashboardView view;

    public ObservadorDashboardController(ObservadorDashboardView view) {
        this.view = view;
    }

    public void handleMenuClick(String opcion) {
        JPanel content = view.getContentArea();
        content.removeAll();

        JLabel label = new JLabel("Vista de solo lectura: " + opcion, SwingConstants.CENTER);
        label.setFont(new Font("Segoe UI", Font.BOLD, 18));
        content.add(label);

        content.revalidate();
        content.repaint();
    }

    public void handleLogout() {
        JOptionPane.showMessageDialog(view, "Cerrando sesión del Observador...");
        System.exit(0);
    }
}
