...
public class OrdenCompraCabeceraDAO {
    ...
    public String obtenerUltimoId() {
        ...
        String sql = "SELECT Id_orden_compra FROM orden_compra_cabecera ORDER BY Id_orden_compra DESC LIMIT 1";
        try (PreparedStatement ps = conexion.getConnection().prepareStatement(sql);
             ResultSet rs = ps.executeQuery()) {
            if (rs.next()) {
                ultimoId = rs.getString("Id_orden_compra");
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    // --- OBTENER POR ID ---
    public OrdenCompraCabecera obtenerPorId(String idOrdenCompra) {
        OrdenCompraCabecera c = null;
        String sql = "SELECT * FROM orden_compra_cabecera WHERE Id_orden_compra = ?";
    ...
    private OrdenCompraCabecera mapearCabecera(ResultSet rs) throws SQLException {
    ...
    // --- SET PARAMS PARA INSERT/UPDATE ---
    private void setParametrosCabecera(CallableStatement cs, OrdenCompraCabecera c) throws SQLException {
        ...
        cs.setObject(15, c.getComNombre(), java.sql.Types.INTEGER);
